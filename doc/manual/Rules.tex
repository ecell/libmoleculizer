...
subelements, which contain the description of the network rule-based model:
'modifications', 'mols', 'allosteric-plexes', 'allosteric-omnis',
'reaction-gens', 'explicit-species', and 'explicit-reactions'.

\lstset{language=XML}
\begin{lstlisting}[caption=Overall structure of a MZR file, label=overallmzrstruct]
...
\section{The 'modifications' Section - /moleculizer-input/model/modifications }
This section consists of a list of zero or more modification elements,
...
Each modification that is defined must have a name and a weight-delta,
the amount by which the modification changes the molecular weight of
the mol that it appears on.  

To represent this information, each 'modification' element has a
mandatory name attribute, and a mandatory child element called
'weight-delta' that itself has a mandatory 'daltons' attribute.  
...
A mod-mol element represents the definition of a single protein type,
or more generally structure-posessing molecule type, in the reaction
network.  A mod-mol is defined with a name, a weight, and any number
of binding and modification sites. These mod-mols and their structural
