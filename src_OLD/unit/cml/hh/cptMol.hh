...
namespace cml
{
  // Base of mol types used in compartmental moleculizer.
  class cptMol :
    public cpx::basicMol<cptBndSite>,
    public fnd::feature<cpx::cxMol<clx::cptPlexSpecies, clx::cptPlexFamily> >
  {
    // Rates of diffusion across compartment boundaries.
  ...
  public:
    ...
    // Ideally, boundaryRates should depend on the state.  This means
    // customizing alloMol, which is already templatically very complicated.
    // (That little inheritance snarl is still waiting to be rationalized.)
    ...
    double
    getBoundaryRate(int boundaryIndex) const
    {
      return boundaryRates[boundaryIndex];
    }
