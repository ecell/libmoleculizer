...
namespace dimer
{
  // Can generate a better error messages if needed here.

  std::string
  missingDimerizeRateXcpt::mkMsg(const plx::cxSite& cxLeft,
  ...
  std::string
  missingDimerizeInvariantXcpt::mkMsg(const plx::cxSite& cxLeft,
				      const plx::cxSite& cxRight)
  {
    std::ostringstream msgStream;
  ...
  std::string
  missingDecomposeRateXcpt::mkMsg(const plx::cxBinding& cx)
  {
    std::ostringstream msgStream;
