...
namespace bnd
{
  std::vector<bindingSite>
  smallMol::makeBindingSites(const std::string& rMolName)
  {
    // Make the one shape of the one binding site.
    siteShape theShape(rMolName);

    // Make mapping of names to shapes.
    std::map<std::string, siteShape> nameToShape;
    nameToShape.insert(std::make_pair(rMolName,
				      theShape));

    // Make the single binding site.
    bindingSite theSite(rMolName,
			nameToShape,
			rMolName);

    // Return vector just containing the one binding site.
    return std::vector<bindingSite>(1, theSite);
  }
  ...
  smallMol::insertElt(xmlpp::Element* pMolsElt) const throw(std::exception)
  {
    ...
    const molState* pDefaultState = getDefaultState();
