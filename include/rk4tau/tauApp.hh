...
#include "rk4tau/rk4tau.hh" // For namespace definition.
#include "rk4tau/dump.hh"
#include "rk4tau/tauParse.hh"

namespace rk4tau
{
  class rejectionLimitXcpt : public mzr::mzrXcpt
  {
    static std::string
  ...
  public:
    rejectionLimitXcpt(int rejectionLimit,
		       double when) :
      mzr::mzrXcpt(mkMsg(rejectionLimit,
			 when))
    {}
  };
    ...
    public domUtils::domBatchApp
  {
    ...
    leap(const std::vector<double>& rGradient,
	 ...
	 int& rTotalReactionCount) throw(mzr::mzrXcpt); // out
  public:
    tauApp(int argc,
	   char** argv,
