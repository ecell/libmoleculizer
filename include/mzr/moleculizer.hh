...
#include "mzr/unit.hh"
#include "domUtils/domBatchJob.hh"


namespace mzr
{
  class insufficientArgsXcpt : public mzrXcpt
  {
  public:
    insufficientArgsXcpt(void) :
      mzrXcpt("Insufficient command-line arguments or "
	      "missing command-line argument.")
    {}
  };

  class badPosIntArgXcpt : public mzrXcpt
  {
    static std::string
  ...
  public:
    badPosIntArgXcpt(const std::string& rTheBadArgument) :
      mzrXcpt(mkMsg(rTheBadArgument))
    {}
  };

  class unrecognizedArgXcpt : public mzrXcpt
  {
    static std::string
    mkMsg(const std::string& rTheUnrecognizedArg) 
    {
      std::ostringstream msgStream;
      msgStream << "Unrecognized command-line argument `"
		<< rTheUnrecognizedArg
		<< "'.";
      return msgStream.str();
    }
  public:
    unrecognizedArgXcpt(const std::string& rTheUnrecognizedArg) :
      mzrXcpt(mkMsg(rTheUnrecognizedArg))
    {}
  };
  ...
  class moleculizer :
    public domUtils::domBatchApp
  {
    // Exceptions thrown during parsing of input.
  ...
  public:
    ...
    moleculizer(void) throw(std::exception);
