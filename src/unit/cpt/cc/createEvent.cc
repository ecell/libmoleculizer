...
namespace cpt
{
  ...
  createEvent(globalSpecies* pSpecies,
    ...
    compartmentPops(pSpecies->getCompartmentGraph().compartments.size(),
  ...
  createEvent(globalSpecies* pSpecies,
    ...
    pSpeciesToCreate(pSpecies),
    compartmentPops(rCompartmentPops)
  {}

  fnd::eventResult
  createEvent::happen(cptApp& rApp)
    throw(std::exception)
  {
    // Update the species in the various compartments given by the counts
    // given in compartmentToCounts.
    fnd::sensitivityList<cptReaction> affectedReactions;
    pSpeciesToCreate->update(compartmentPops,
			     affectedReactions,
			     rApp.getCptUnit().getGenerateDepth());

    // Adjust the affected reactions' propensities.
