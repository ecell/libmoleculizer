...
#include "mzr/mzrUnitParse.hh" // For unknownSpeciesXcpt, unknownDumpableXcpt..
#include "mzr/physConst.hh"
#include "rk4tau/tauEltName.hh"
#include "rk4tau/dump.hh"  // For tauDumpable, dumpStream.
#include "rk4tau/tauApp.hh"
#include "rk4tau/tauParse.hh"

namespace rk4tau
{
  class duplicateSpeciesEntryXcpt : public mzr::mzrXcpt
  {
    static std::string
  ...
  public:
		     ...
		     const std::string& rDumpableName) const
    {
      ...
      if(iEntry == end()) throw mzr::unknownDumpableXcpt(pRequestingNode,
						    rDumpableName);
  ...
  public:
    ...
    mustFindSpecies(xmlpp::Node* pRequestingNode,
      ...
      throw(mzr::unknownSpeciesXcpt)
    {
      const_iterator iEntry = find(rSpeciesName);
      if(iEntry == end()) throw mzr::unknownSpeciesXcpt(pRequestingNode,
							rSpeciesName);
      return iEntry->second;
    }
  };
  ...
  public:
	...
	= domUtils::mustGetAttrString(pSpeciesEntryElt,
      ...
      int population
	= domUtils::mustGetAttrInt(pSpeciesEntryElt,
    ...
    pAdaptor(0)
  {
    ...
    molarConst = volume * AVOGADROS_NUMBER;
    ...
    dumpInterval
      = domUtils::mustGetAttrDouble(pDumpIntervalElt,
    ...
    xmlpp::Element* pEpsilonElt
      = domUtils::mustGetUniqueChild(pRootElt,
				     tauEltName::epsilon);
    epsilon
      = domUtils::mustGetAttrDouble(pEpsilonElt,
    ...
    xmlpp::Element* pSeedElt
      = domUtils::mustGetUniqueChild(pRootElt,
				     tauEltName::seed);
    seedString
      = domUtils::mustGetAttrString(pSeedElt,
