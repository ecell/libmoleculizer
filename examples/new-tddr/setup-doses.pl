...
($input_file_path, $tab_values_file, $program_name) = @ARGV;
...
open(VALUE_HANDLE, $tab_values_file);
$_ = <VALUE_HANDLE>;
@sub_targets = split;

# The first header, on the output directory column, is a dummy.
shift @sub_targets;
...
while(<VALUE_HANDLE>)
{
    ...
    # Make the simulation output directory.
    $sim_dir = "$input_file_dir/$sim_dir_relative_path";
    mkdir $sim_dir
