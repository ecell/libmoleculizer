...
namespace cml
{
  ...
  cptModMol(const std::string& rName,
    ...
    cpx::modMol<cptMol>(cptMol(rName,
			       rSites,
			       rBoundaryRates),
			molecularWeight,
			rDefaultModMap)
		      ...
		      cpx::molParam param) const
  {
    ...
    xmlpp::Element* pModMolInstanceRefElt
      = pInstanceStatesElt->add_child(clx::eltName::modMolInstanceRef);

    pModMolInstanceRefElt
      ->set_attribute(clx::eltName::modMolInstanceRef_nameAttr,
  ...
  // Class for inserting a binding site on a cptModMol.
  class insertSite :
    public std::unary_function<cptBndSite, void>
  {
  ...
  public:
    ...
    void
    operator()(const cptBndSite& rSite) const throw(std::exception)
    {
  ...
  xmlpp::Element*
  cptModMol::insertElt(xmlpp::Element* pMolsElt) const
    throw(utl::xcpt)
  {
    ...
    //
    // Distressingly, cptModMol gets "begin" and "end" two different ways:
    // as a basicMol and as a feature.
    std::for_each(cpx::basicMol<cptBndSite>::begin(),
		  cpx::basicMol<cptBndSite>::end(),
