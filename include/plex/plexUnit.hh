...
namespace plx
{
  ...
  class plexUnit : public mzr::unit
  {
  public:
    typedef int molSiteSpec;
    typedef std::pair<bnd::mol*, molSiteSpec> structuralSite;
    typedef std::pair<structuralSite, structuralSite> structuralBinding;
    typedef std::map<structuralBinding, plx::bindingFeature> bindingFeatureMap;
  ...
  private:
    ...
    //
    // This function is purely structural, unlike the mapping of %pairs
    // of site params to binding params.  That map is used to take the
    // internal state of molecules into account when computing binding
    // parameters.  It's assumed now that binding parameters are
    // determined by the shapes (siteParams) assigned to the sites that
    // are bound together.
    //
    ...
    // participates in.
    //
    // This was formerly in the dimerUnit.
    bindingFeatureMap bindingFeatures;

    // These catalogs are needed to pick out plexDumpables and
    // omniPlexDumpables for use by the inventoryTaggedCmd and
    // inventoryOmniCmd.
//     mzr::catalog<mzr::paramDumpable<plexSpecies> > plexDumpables;
//     mzr::catalog<omniPlexDumpable> omniPlexDumpables;
  ...
  public:
    ...
    bindingFeature*
    addBindingFeature(bnd::mol* pLeftMol,
		      int leftMolSiteSpec,
		      bnd::mol* pRightMol,
    ...
    plx::bindingFeature*
    findBindingFeature(bnd::mol* pLeftMol,
		       int leftMolSiteSpec,
		       bnd::mol* pRightMol,
		       int rightMolSiteSpec);
    ...
    // that must be made.
    std::set<plexFamily*> omniPlexFamilies;

    // For use by omniPlex parsing code.
    void
    addOmniPlexFamily(plexFamily* pPlexFamily)
    {
      omniPlexFamilies.insert(pPlexFamily);
    }

    // For use by plexFamily::connectToFeatures.
    const std::set<plexFamily*>&
    ...
    void
    addPlexQuery(plexQuery* pQuery)
    {
      plexQueries.push_back(pQuery);
    }

    void
    addStructureQuery(omniStructureQuery* pQuery)
    {
...
//     }
    ...
    parseDomInput(xmlpp::Element* pRootElt,
		  ...
		  xmlpp::Element* pEventsElt) throw(mzr::mzrXcpt);
  
    void
    insertStateElts(xmlpp::Element* pRootElt) throw(mzr::mzrXcpt);
