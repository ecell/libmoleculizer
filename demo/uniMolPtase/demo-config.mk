...
$(DEMO_DIR)/sbml : $(OUTPUT_DIR)/state-dump.sbml \
	$(OUTPUT_DIR)/state-dump-pretty.sbml

$(OUTPUT_DIR) :
	mkdir $@

# Run the simulation, which generates a state dump.
...
$(OUTPUT_DIR)/state-dump.sbml : $(OUTPUT_DIR)/simulation-done
	state2sbml $(@D)/state-dump.xml $@

# Pretty-print (indent) the sbml.
$(OUTPUT_DIR)/state-dump-pretty.sbml : $(OUTPUT_DIR)/state-dump.sbml
	xmlpretty $< $@

# To do multiple runs of the demo.
