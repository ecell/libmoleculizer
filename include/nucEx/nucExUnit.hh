...
namespace nucEx
{
  class nucExUnit : public mzr::unit
  {
  public:
    ...
    nucExUnit(mzr::moleculizer& rMoleculizer,
    ...
    {
      // This unit basically provides only reaction generators.
      inputCap.addReactionGenName(eltName::nucleotideExchangeGen);
      inputCap.addReactionGenName(eltName::autoHydrolysisGen);
      inputCap.addReactionGenName(eltName::heteroHydrolysisGen);

      // Register omniplex nodes for processing by the plexUnit.
      // These are "enabling complexes" for reaction generators.
      const std::string slash("/");
      std::ostringstream nucExGensXpath;
      nucExGensXpath << mzr::eltName::model
		     << slash
		     ...
		     << slash
		     << plx::eltName::plex;
      rPlexUnit.addOmniXpath(nucExGensXpath.str());

      std::ostringstream hetHydGensXpath;
      hetHydGensXpath << mzr::eltName::model
		      << slash
		      << mzr::eltName::reactionGens
		      << slash
		      << eltName::heteroHydrolysisGen
		      << slash
		      << plx::eltName::plex;
      rPlexUnit.addOmniXpath(hetHydGensXpath.str());
    }
