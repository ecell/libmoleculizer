...
namespace cpt
{
  void
  parseDumpStateEvent::
  operator()(xmlpp::Node* pDumpStateEventNode) const
    throw(utl::xcpt)
  {
    ...
    // Construct the dump event and register for memory management.
    dumpStateEvent* pDumpStateEvent
      = new dumpStateEvent(fileName);
    rCptUnit.addUserEvent(pDumpStateEvent);
    ...
    if(now <= time)
      {
	rApp.getQueue().scheduleEvent(pDumpStateEvent,
				      time);
      }
  }
}
