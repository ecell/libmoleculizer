...
#include "stoch/stochXcpt.hh"
#include "stoch/stochUnit.hh"

namespace stoch
{
  bool
  stochUnit::addStochSpecies(const std::string& rSpeciesName,
  ...
  stochSpecies*
  stochUnit::findStochSpecies(const std::string& rSpeciesName)
  {
    return stochSpeciesByName.findEntry(rSpeciesName);
  }

  stochSpecies*
  stochUnit::mustGetStochSpecies(xmlpp::Node* pRequestingNode,
				 const std::string& rSpeciesName)
    throw(unknownStochSpeciesXcpt)
  {
    stochSpecies* pSpecies = findStochSpecies(rSpeciesName);
    if(0 == pSpecies) throw unknownStochSpeciesXcpt(pRequestingNode,
						    rSpeciesName);
    return pSpecies;
  }

  void
  stochUnit::addNoSubstrateArrow(mzr::reaction* pReaction)
  {
    noSubstrateArrows.push_back(pReaction);
  }

  class insertStochSpeciesElt :
    public std::unary_function<mzr::autoCatalog<stochSpecies>::value_type, void>
  {
  ...
  void
  stochUnit::insertStateElts(xmlpp::Element* pRootElt) throw(std::exception)
  {
