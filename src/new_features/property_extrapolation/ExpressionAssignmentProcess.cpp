...
public:
    ...
    virtual void initialize()
    {
        ...
        for ( VariableReferenceVector::const_iterator                  i( getVariableReferenceVector().begin() );
                i != getVariableReferenceVector().end(); ++i )
        {
            if ( i->getCoefficient() != 0 )
            {
                this->variableReference = *i;
            }
        }
    }
    ...
    virtual void fire()
    {
        this->variableReference.setValue
        ( this->variableReference.getCoefficient() *
          this->virtualMachine.execute( compiledCode ) );
    }
