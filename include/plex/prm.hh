...
namespace plx
{
  ...
  public:
    ...
    // though.
    const bnd::siteParam&
    getSiteParam(const plexSiteSpec& rSpec) const
    {
      siteToShapeMap::const_iterator iSiteEntry
	= siteParams.find(rSpec);
      
      if(siteParams.end() == iSiteEntry)
	throw mzr::mzrXcpt("plexParam::getSiteParam: no such entry.");
      else return iSiteEntry->second;
    }

    // Need both constant and non-constant versions of this.
    // Is there a better way than writing the code twice?
    bnd::siteParam&
    getSiteParam(const plexSiteSpec& rSpec)
    {
      std::map<plexSiteSpec, bnd::siteParam>::iterator iSiteEntry
	= siteParams.find(rSpec);
