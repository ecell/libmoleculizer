...
namespace clx
{
  ...
  public:
	...
	= pBindingElt->add_child(eltName::molInstanceRef);
      ...
      const cml::cptMol& rMol = *(rPlx.mols[rSiteSpec.molNdx()]);
	...
	= pMolInstanceRefElt->add_child(cml::eltName::bindingSiteRef);

      const cml::cptBndSite& rBindingSite
	= rMol[rSiteSpec.siteNdx()];

      pBindingSiteRefElt->set_attribute(cml::eltName::bindingSiteRef_nameAttr,
