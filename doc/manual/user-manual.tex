%% Some basic substitution commands I'm defining. 
\newcommand{\libmzrwebsite}{\emph{http://sourceforge.net/projects/moleculizer/}}
\newcommand{\myemail}{\emph{addy@molsci.org}}
\newcommand{\currentversion}{1.0 }


%% The libmoleculizer manual.

\documentclass{report}

\usepackage{listings}
\usepackage{makeidx}
\usepackage{setspace}

% \usepackage{glossary}

%% Some definitions for the listings package.
\lstloadlanguages{XML, C++, C, Python}
\lstnewenvironment{LocalCPP}{\lstset{language=C++,caption=,label=,keywords=}}{}
\lstnewenvironment{LocalRNG}{\lstset{language=XML, keywords=} }{}
\lstnewenvironment{ExampleXML}{\lstset{language=XML, caption=,
    label=,showspaces=false, showtabs=false, keywords=} }{}
\lstnewenvironment{ShellCommands}{\lstset{language=Shell,caption=,label=, keywords=}}{}

\author{Nathan Addy}
\title{Libmoleculizer \currentversion User's Manual}

% I'm told these must be in the preamble.
\makeindex
\doublespacing

% \makeglossary

\begin{document}

\maketitle

\tableofcontents


%% Each of the chapters, in order. 
\include{Introduction}
\include{ConceptualOverview}
\include{Installing}
\include{Using}
\include{Interfaces}
\include{Rules}
\include{Cookbook}
\include{APIReference}
\include{MZRReference}

%% Print the glossary, the index, and then the bibliography.
% \printglossary
\printindex

\bibliographystyle{plain}
\bibliography{references}

\end{document}
