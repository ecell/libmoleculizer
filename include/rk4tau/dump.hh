...
#include "rk4tau.hh"  // For namespace definition.

namespace rk4tau
{
  // Thrown by dumpStream::mustSetStream.
  class badFileNameXcpt : public mzr::mzrXcpt
  {
    static std::string
  ...
  public:
    badFileNameXcpt(xmlpp::Node* pRequestingNode,
		    const std::string& rBadFileName) :
      mzr::mzrXcpt(mkMsg(pRequestingNode,
		    rBadFileName))
    {}
  };
  ...
  public:
    ...
    void
    addDumpable(const tauDumpable& rDumpable)
    {
      push_back(rDumpable);
    }

    void
    dump(const std::vector<int>& rPops,
