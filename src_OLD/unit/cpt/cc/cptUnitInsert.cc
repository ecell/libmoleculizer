...
//   };
  ...
  class insertFamilyReactions :
    public std::unary_function<const std::vector<globalReaction>*, void>
  {
  ...
  public:
    ...
    void
    operator()(const std::vector<globalReaction*>* pFamily) const 
      throw(std::exception)
    {
      std::for_each(pFamily->begin(),
		    pFamily->end(),
		    std::bind2nd(std::mem_fun(&globalReaction::insertElt),
				 pTagReactionsElt));
    }
  };

  class insertExplicitSpeciesTag :
    public std::unary_function<utl::catalog<globalSpecies>::value_type, void>
  {
...
//   };
    ...
    throw(std::exception)
  {
    ...
    // Insert the compartment graph, including the volumes of the compartments.
    getCompartmentGraph().insertState(pModelElt);
    ...
    double time = rCptApp.getSimTime();
    ...
    std::for_each(speciesStreams.begin(),
		  ...
		  std::bind2nd(std::mem_fun
			       (&cptSpeciesStream::insertDumpedSpeciesTags),
			       pTaggedSpeciesStreamsElt));
    ...
    std::for_each(dumpStreams.begin(),
		  dumpStreams.end(),
		  std::bind2nd(std::mem_fun
			       (&cptDumpStream::insertTaggedDumpStreamElts),
