...
(let ((options (get-options)))
  (let ((input-file-pair (string-map-ref options
	...
	(randomize-fraction-pair (string-map-ref options
						 "-fraction")))
    (or input-file-pair
	(thread-error "randomize-rates.scm: no -input option given."))
    (or output-file-pair
	(thread-error "randomize-rates.scm: no -output option given."))
    (or randomize-count-pair
	(thread-error "randomize-rates.scm: no -count option given."))
    (or randomize-fraction-pair
	(thread-error "randomize-rates.scm: no -fraction option given."))
    ...
    (let ((count (let ((count-port (open-input-string
	  ...
	  (fraction (let ((fraction-port (open-input-string
			...
			fraction))))
      (randomize-rates (cdr input-file-pair)
