...
open(RESPONSES, $responses_file) or
    die("write-gp-dora-plot-script.pl: could not open "
...
print SCRIPT "set title \"Normalized dose-response curves\"\n";
print SCRIPT "set xlabel \"Number of dose molecules\"\n";
print SCRIPT "set ylabel \"Normalized response\"\n";
print SCRIPT "plot \\\n";
...
while(@rest_titles)
{
    ...
    print SCRIPT
	"\"$histo_dir/all-drs\" using 1:2 smooth bezier with lines title \"$title\"";
