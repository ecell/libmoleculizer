...
namespace plx
{
  ...
  int
  parserPlex::addMolByName(const std::string& rName,
			   bnd::mol* pMol)
  {
  ...
  int
  parserPlex::getMolNdxByName(const std::string& rName) const
  {
  ...
  int
  parserPlex::mustGetMolNdxByName(xmlpp::Node* pRequestingNode,
				  const std::string& rInstanceName) const
    throw(unknownMolInstanceXcpt)
  {
    int ndx = getMolNdxByName(rInstanceName);
    if(ndx < 0) throw unknownMolInstanceXcpt(pRequestingNode,
					     rInstanceName);
    return ndx;
  }
